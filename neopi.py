...
#         Wayne Thursby (https://github.com/thursby)
#
...
class LanguageIC:
   ...
   """
    ...
    def calculate(self, data, filename):
        ...
        ic = float(char_count) / (total_char_count * (total_char_count
                                  - 1))
    ...
    def printer(self, count):
       ...
       # Calculate the Total IC for a Search
        ...
        for x in range(count):
            print ' {0:>7.4f}        {1}'.format(self.results[x]['value'
                    ], self.results[x]['filename'])
        return
...
class FileMD5Sum:
    ...
    def calculate(self, data, filename):
        ...
        md5 = md5sum(data)
        self.results.append({'filename': filename, 'value': md5})
        return md5
    ...
    def printer(self, count):
        ...
        print '\n[[ Top %i md5 files for a given search ]]' % count
        if count > len(self.results):
            count = len(self.results)
        for x in range(count):
            print ' {0}        {1}'.format(self.results[x]['value'],
                    self.results[x]['filename'])
        return
...
class LongestWord:
    ...
    def printer(self, count):
        ...
        if count > len(self.results):
            count = len(self.results)
        for x in range(count):
            print ' {0:>7}        {1}'.format(self.results[x]['value'],
                    self.results[x]['filename'])
        return
...
class SignatureNasty:
    ...
    def calculate(self, data, filename):
       ...
       # Lots taken from the wonderful post at http://stackoverflow.com/questions/3115559/exploitable-php-functions
        ...
        valid_regex = \
            re.compile('(eval\(|file_put_contents|base64_decode|python_eval|exec\(|passthru|popen|proc_open|pcntl|assert\(|system\(|shell)'
                       , re.I)
...
class SignatureSuperNasty:
    ...
    def calculate(self, data, filename):
        if not data:
            return ('', 0)
        valid_regex = re.compile('(@\$_\[\]=|\$_=@\$_GET|\$_\[\+""\]=)'
                                 , re.I)
        matches = re.findall(valid_regex, data)
        self.results.append({'filename': filename,
                            'value': len(matches)})
        ...
        for (root, dirs, files) in os.walk(args[0]):
            for file in files:
                ...
                if valid_regex.search(file) \
                    and os.path.getsize(filename) > SMALLEST:
                    try:
                        data = open(root + '/' + file, 'rb').read()
                    except:
                        data = False
                        print 'Could not read file :: %s/%s' % (root,
                                file)
                    yield (data, filename)
...
if __name__ == '__main__':
   ...
   """
    ...
    parser = \
        OptionParser(usage='usage: %prog [options] <start directory> <OPTIONAL: filename regex>'
                     , version='%prog 1.0')
    parser.add_option(
        '-c',
        '--csv',
   ...
   # Error on an invalid path
    ...
    if options.is_auto:
        valid_regex = \
            re.compile('(\.php|\.asp|\.aspx|\.scath|\.bash|\.zsh|\.csh|\.tsch|\.pl|\.py|\.txt|\.cgi|\.cfm|\.htaccess)$'
                       )
        ...
        if data:
           ...
           # a row array for the CSV
            ...
            if options.ignore_unicode:
                ...
                fileAsciiHighRatio = float(asciiHighCount) \
                    / float(len(data))

            if options.ignore_unicode == False or fileAsciiHighRatio \
                < .1:
                for test in tests:
   ...
   # Print top rank lists
    ...
    for x in range(count):
        print ' {0:>7}        {1}'.format(rank_sorted[x][1],
                rank_sorted[x][0])
